...
const TopPanel = () => {
  ...
  const getCurrentPathLink = ():JSX.Element[] => {
    ...
    if(currentRoute.length === 1){
      ...
      let mainPath = currentRoute.split(new RegExp('\\?'))[0]
      mainPath.split(new RegExp('/')).map((p, i, arr) =>  {
        if( i === arr.length - 1){
          res.push(<span className='top-panel_current-link' key={i}>{pathDictionary[p]}</span>)
        } else {
          res.push(<a className='top-panel_link' key={i} href={'/'+p}>{pathDictionary[p]}</a>)
        }
  ...
  return (
    <section className="top-panel">
      <div className="top-panel_text">
        ...
        { currentRoute.length > 1 ? <span><FontAwesomeIcon icon={faLocationCrosshairs} /> </span> : ''}    
